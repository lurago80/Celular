        ...
        for item in venda.items.all():
            ...
            if item.produto.tipo == 'celular':
                ...
                if not imei or len(imei) != 15:
                    messages.error(
                        request,
                        f'IMEI obrigatório para {item.produto.nome} (deve ter 15 dígitos)'
                    )
                    return redirect('core:finalizar_venda', venda_id=venda_id)
...
def pdv_finalizar(request):
    ...
    try:
        ...
        desconto_percentual = request.GET.get('desconto', '0')
        forma_pagamento = request.GET.get('pagamento', 'dinheiro')
        
        try:
            venda.desconto_percentual = Decimal(str(desconto_percentual))
        except:
...
def termo_garantia_conserto(request, os_id):
    ...
    # Calcula data fim da garantia a partir da data de conclusão ou entrada
    data_base = os_obj.data_conclusao or os_obj.data_entrada.date()
...
def busca_imei(request):
    """Busca rápida por IMEI: encontra vendas e OSs associadas"""
    ...
    if imei:
        if not imei.isdigit() or len(imei) != 15:
            erro = 'IMEI deve conter exatamente 15 dígitos numéricos.'
        else:
            garantia = (
                Garantia.objects
                .filter(imei=imei)
            ...
            if not garantia and not os_resultado:
                erro = f'Nenhum registro encontrado para o IMEI {imei}.'
